...
public class ChiTietBaiDang_ThongBao extends AppCompatActivity {
    ImageView imgNha;
    ...
    private int mInterval = 2555; // 5 seconds by default, can be changed later
    ...
    ScrollView scroll;
    Geocoder coder ;
    String so,duong,quan,tp,idnha_nhantb,idnct_nhantb;
    ...
    private void onMyMapReady(GoogleMap googleMap) {
        ...
        ggmap.setOnMapLoadedCallback(new GoogleMap.OnMapLoadedCallback() {
            ...
            public void onMapLoaded() {
                    try {
                        ArrayList<Address> adresses = (ArrayList<Address>) coder.getFromLocationName(so + duong +" ,"+ tp,1);
                        if(adresses.size()!=0) {
                            longitude = adresses.get(0).getLongitude();
                            latitude = adresses.get(0).getLatitude();
                            LatLng house = new LatLng(latitude,longitude);
                            ggmap.addMarker(new MarkerOptions().position(house).title(so+" "+duong+","+quan+","+tp)).showInfoWindow();
    ...
    private void MoFormGiaSH(){
        Intent i  = new Intent(ChiTietBaiDang_ThongBao.this, GiaSH_Viewer.class);
        Bundle b = new Bundle();
    ...
    private void NhanDuLieu(){
        ...
        idnha_nhantb = b.getString("idnha");
        idnct_nhantb = b.getString("idnct");
        for(int j=0;j<arrNha.size();j++){
            if(arrNha.get(j).getId().compareTo(idnha_nhantb)==0){
                nha = arrNha.get(j);
            }
        }
    ...
    private void AnhXa() {
        ...
        LinearLayoutManager linearLayoutManager2 = new LinearLayoutManager(ChiTietBaiDang_ThongBao.this, LinearLayoutManager.VERTICAL, false);
        reView_DSPhong.setLayoutManager(linearLayoutManager2);
        adapter_dsNhaCT = new Adapter_DSNhaCT(arrAnhNCT,arrNCT, getApplicationContext(),ChiTietBaiDang_ThongBao.this);
        ...
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(ChiTietBaiDang_ThongBao.this, LinearLayoutManager.VERTICAL, false);
        ...
        LinearLayoutManager linearLayoutManager3 = new LinearLayoutManager(ChiTietBaiDang_ThongBao.this, LinearLayoutManager.VERTICAL, false);
    ...
    public void Progress(){
        progressBar = new ProgressDialog(ChiTietBaiDang_ThongBao.this);
        progressBar.show();
